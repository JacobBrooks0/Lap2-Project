const showSkills = async () => {
  ...
  const resp = await fetch(`https://florinate-api.onrender.com/classes`);
  if (!resp.ok) {
  ...
  allSkillClasses.forEach(async (skillClass) => {
    const { class_id, name, info, main_image_url, start_date, end_date } =
      skillClass;
    ...
    const resp3 = await fetch(
      `https://florinate-api.onrender.com/classes/${class_id}/is-at-capacity`
    );
    ...
    const checkIfEnrolled = () => {
      if (enrolledClasses) {
        const enrolled = enrolledClasses.find(
          (enrolledClass) => enrolledClass.class_id == skillClass.class_id
        );
        return enrolled;
      }
    };
    ...
    classDate.innerHTML = `${new Date(start_date * 1000).toLocaleString(
      "en-GB",
      {
        day: "numeric",
        month: "long",
...
const applyToClass = async (skillClass) => {
  ...
  try {
    ...
    const resp = await fetch(
      `https://florinate-api.onrender.com/classes/${class_id}/enroll`,
      options
    );
...
const leaveClass = async (skillClass) => {
  try {
    ...
    const resp = await fetch(
      `https://florinate-api.onrender.com/classes/${skillClass.class_id}/delist`,
      options
    );
    if (!resp.ok) {
      console.log(Error.detail);
